...
import type { OptionsMap, OptionDef } from '@/lib/options-map.lib.js';
...
/**
 * Checks that all required option definitions are satisfied by the options map.
 *
 * Returns a formatted user-facing error string when any required option is absent,
 * or null when the command may proceed.
 *
 * Output format:
 *   ❌ Missing required input
 *
 *   Usage:
 *   {prefix}{command} key:<your key> optKey:<optional optKey>
 *
 *   Fields:
 *   • key (required) – description
 *   • optKey (optional) – description
 */
export function validateOptions(
  ...
  if (!optionDefs.length) return null;

  const missing = optionDefs.filter(
    (o) => (o.required ?? false) && !options.has(o.name),
  );
  if (!missing.length) return null;

  // Build the usage line: one token per option, placeholder communicates required vs optional
  const usageParts = optionDefs
    .map((o) => {
      const placeholder = o.required
        ? `<your ${o.name}>`
        : `<optional ${o.name}>`;
  ...
  const fieldLines = optionDefs.map((o) => {
    const label = o.required ? 'required' : 'optional';
    return `• ${o.name} (${label}) – ${o.description ?? 'No description.'}`;
  });

/**
 * /jokeoverhead — Joke Over Head Overlay on a Photo or User's Avatar
 *
 ...
 *
 * The image is passed to the PopCat /v2/jokeoverhead endpoint and returned as a
 * Buffer attachment.
 *
...
export const config: CommandConfig = {
  ...
  role: Role.ANYONE,
  author: 'AjiroDesu',
  description: "The joke went over a photo or a user's head.",
  category: 'fun',
  usage: [
...
export const nonAdminGuide: string = [
  '💨 **How to use /jokeoverhead (FB Page non-admin only):**',
  '1️⃣  Send a photo in the conversation (tap the photo/camera icon).',
  '2️⃣  Reply to that photo with the command: `!jokeoverhead`',
...
export const onCommand = async ({
  ...
  try {
    ...
    const base = createUrl('popcat', '/v2/jokeoverhead');
    if (!base) throw new Error('Failed to build Joke Overhead API URL.');
    ...
    await chat.replyMessage({
      ...
      message: '💨 **Went right over their head**',
      attachment: [
        {
          name: 'jokeoverhead.png',
          stream: Buffer.from(await res.arrayBuffer()),
        },
      ],
    });
  } catch (err) {

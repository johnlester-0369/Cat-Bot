 ...
 *
 * The image is passed to the Wajiro /api/v1/jo endpoint (param: image)
 * and returned as a Buffer attachment.
 */
...
export const config: CommandConfig = {
  ...
  role: Role.ANYONE,
  author: 'AjiroDesu',
  description: "Apply the Jojo effect to a photo or user's avatar.",
  category: 'fun',
  usage: [
...
export const onCommand = async ({
  ...
  try {
    ...
    // Note: this endpoint uses 'image' as the query param, not 'avatar'
    const url = createUrl('wajiro', '/api/v1/jo', { image: imageUrl });
    ...
    await chat.replyMessage({
      style: MessageStyle.MARKDOWN,
      message: '🌀 **Jojo Effect**',
      attachment: [{ name: 'jojo.png', stream: imageBuffer }],
    });
  } catch (err) {

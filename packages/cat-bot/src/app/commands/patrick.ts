 ...
 *
 * The image is passed to the Wajiro /api/v1/patrick endpoint and returned
...
export const config: CommandConfig = {
  ...
  role: Role.ANYONE,
  author: 'AjiroDesu',
  description: "Apply the Patrick Star effect to a photo or user's avatar.",
  category: 'fun',
  usage: [
...
export const onCommand = async ({
  ...
  try {
    ...
    const url = createUrl('wajiro', '/api/v1/patrick', { avatar: imageUrl });
    ...
    await chat.replyMessage({
      style: MessageStyle.MARKDOWN,
      message: '⭐ **Patrick Star Effect**',
      attachment: [{ name: 'patrick.png', stream: imageBuffer }],
    });
  } catch (err) {

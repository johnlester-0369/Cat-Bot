 ...
 *
 * The image is passed to the PopCat /v2/wanted endpoint and returned as a
 * Buffer attachment.
 *
...
export const config: CommandConfig = {
  ...
  role: Role.ANYONE,
  author: 'AjiroDesu',
  description: 'Generate a wanted poster using a photo or avatar.',
  category: 'fun',
  usage: [
...
export const nonAdminGuide: string = [
  '🤠 **How to use /wanted (FB Page non-admin only):**',
  '1️⃣  Send a photo in the conversation (tap the photo/camera icon).',
  '2️⃣  Reply to that photo with the command: `!wanted`',
...
export const onCommand = async ({
  ...
  try {
    ...
    if (attachedImageUrl) {
      // Non-page-admin FB Page path: use the uploaded photo directly
    ...
    const base = createUrl('popcat', '/v2/wanted');
    if (!base) throw new Error('Failed to build Wanted API URL.');
    ...
    await chat.replyMessage({
      ...
      message: '🤠 **Wanted!**',
      attachment: [{ name: 'wanted.png', stream: imageBuffer }],
    });
  } catch (err) {

/**
 * /invert — Invert Colors of a Photo or User's Avatar
 *
 ...
 *
 * The image is passed to the PopCat /v2/invert endpoint and returned as a
 * Buffer attachment.
 *
...
export const config: CommandConfig = {
  ...
  role: Role.ANYONE,
  author: 'AjiroDesu',
  description: "Invert the colors of a photo or a user's avatar.",
  category: 'fun',
  usage: [
...
export const nonAdminGuide: string = [
  '🔄 **How to use /invert (FB Page non-admin only):**',
  '1️⃣  Send a photo in the conversation (tap the photo/camera icon).',
  '2️⃣  Reply to that photo with the command: `!invert`',
...
export const onCommand = async ({
  ...
  try {
    ...
    const base = createUrl('popcat', '/v2/invert');
    if (!base) throw new Error('Failed to build Invert API URL.');

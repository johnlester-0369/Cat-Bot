/**
 * /communism — Communism Overlay
 *
 * Fetches the target user's avatar (mention → replied-to user → self) and
 * passes it to the PopCat /v2/communism endpoint. The API returns an image
...
export const config: CommandConfig = {
  ...
  role: Role.ANYONE,
  author: 'AjiroDesu',
  description: 'Apply a communism overlay to a user\'s avatar.',
  category: 'fun',
  usage: '[@user]',
  cooldown: 5,
  hasPrefix: true,
...
export const onCommand = async ({ chat, user, event }: AppCtx): Promise<void> => {
  ...
  try {
    ...
    const base = createUrl('popcat', '/v2/communism');
    if (!base) throw new Error('Failed to build Communism API URL.');
    ...
    await chat.replyMessage({
      ...
      message: '☭ **Communism!**',
      attachment: [{ name: 'communism.png', stream: imageBuffer }],
    });
  } catch (err) {

/**
 * /cosplayimg — Random Cosplay Image (Updated)
 *
 ...
 * Refresh button always gives a brand-new random cosplay photo.
 */
...
export const config: CommandConfig = {
  name: 'cosplayimg',
  aliases: ['cosplay-image'] as string[],
  version: '2.0.0',
  role: Role.ANYONE,
  author: 'AjiroDesu',
  description: 'Fetch a random cosplay image (direct image response).',
  category: 'random',
  usage: '',
  cooldown: 5,
  hasPrefix: true,
...
async function sendCosplayImg(ctx: AppCtx): Promise<void> {
  const { chat, native, event, button: btn } = ctx;
  const isButtonAction = event['type'] === 'button_action';

  try {
    const url = createUrl('nekolabs', '/random/cosplay');
    if (!url) throw new Error('Failed to build Cosplay API URL.');
    ...
    const payload = {
      ...
      message: '**📸 Random Cosplay Image**',
      attachment: [{ name: 'cosplay.jpg', stream: imageBuffer }],

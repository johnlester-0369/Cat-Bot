/**
 * /greyscale — Apply Greyscale to a Photo or User's Avatar
 *
 ...
 *
 * The image is passed to the PopCat /v2/greyscale endpoint and returned as a
 * Buffer attachment.
 *
...
export const config: CommandConfig = {
  name: 'greyscale',
  aliases: ['grayscale'] as string[],
  version: '1.0.0',
  role: Role.ANYONE,
  author: 'AjiroDesu',
  description: 'Apply a greyscale filter to a photo or a user\'s avatar.',
  category: 'fun',
  usage: [
...
export const nonAdminGuide: string = [
  '🩶 **How to use /greyscale (FB Page non-admin only):**',
  '1️⃣  Send a photo in the conversation (tap the photo/camera icon).',
  '2️⃣  Reply to that photo with the command: `!greyscale`',
...
export const onCommand = async ({
  ...
  try {
    ...
    const base = createUrl('popcat', '/v2/greyscale');
    if (!base) throw new Error('Failed to build Greyscale API URL.');
    ...
    await chat.replyMessage({
      ...
      message: '🩶 **Greyscale**',
      attachment: [{ name: 'greyscale.png', stream: imageBuffer }],
    });
  } catch (err) {

 ...
 *
 * Accepts a text prompt from args and passes it to the PopCat /v2/supreme
 * endpoint. The API returns a Supreme-style logo image sent as a Buffer
 * attachment.
 *
...
export const onCommand = async ({ chat, args, usage }: AppCtx): Promise<void> => {
  ...
  try {
    const base = createUrl('popcat', '/v2/supreme');
    if (!base) throw new Error('Failed to build Supreme API URL.');
    ...
    await chat.replyMessage({
      ...
      message: '🔴 **Supreme**',
      attachment: [{ name: 'supreme.png', stream: imageBuffer }],
    });
  } catch (err) {

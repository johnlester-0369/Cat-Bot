/**
 * /uncover — Uncover a Photo or User's Avatar
 *
 ...
 *
 * The image is passed to the PopCat /v2/uncover endpoint and returned as a
 * Buffer attachment.
 *
...
export const config: CommandConfig = {
  ...
  role: Role.ANYONE,
  author: 'AjiroDesu',
  description: "Uncover a photo or a user's avatar.",
  category: 'fun',
  usage: [
...
export const nonAdminGuide: string = [
  '🔍 **How to use /uncover (FB Page non-admin only):**',
  '1️⃣  Send a photo in the conversation (tap the photo/camera icon).',
  '2️⃣  Reply to that photo with the command: `!uncover`',
...
export const onCommand = async ({
  ...
  try {
    ...
    const base = createUrl('popcat', '/v2/uncover');
    if (!base) throw new Error('Failed to build Uncover API URL.');
    ...
    await chat.replyMessage({
      ...
      message: '🔍 **Uncovered!**',
      attachment: [{ name: 'uncover.png', stream: imageBuffer }],
    });
  } catch (err) {

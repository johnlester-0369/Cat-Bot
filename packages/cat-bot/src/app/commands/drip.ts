 ...
 * passes it to the PopCat /v2/drip endpoint. The API returns an image
...
export const config: CommandConfig = {
  ...
  role: Role.ANYONE,
  author: 'AjiroDesu',
  description: 'Give a user\'s avatar the drip treatment.',
  category: 'fun',
  usage: '[@user]',
  cooldown: 5,
  hasPrefix: true,
...
export const onCommand = async ({ chat, user, event }: AppCtx): Promise<void> => {
  ...
  try {
    ...
    const base = createUrl('popcat', '/v2/drip');
    if (!base) throw new Error('Failed to build Drip API URL.');
    ...
    await chat.replyMessage({
      ...
      message: '💧 **Drip!**',
      attachment: [{ name: 'drip.png', stream: imageBuffer }],
    });
  } catch (err) {

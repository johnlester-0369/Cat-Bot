/**
 * /bluearchive — Random Cat Image (Updated)
 *
 * Now uses the centralized `createUrl` helper (nekolabs registry).
...
export const config: CommandConfig = {
  name: 'bluearchieve',
  aliases: ['ba'] as string[],
  version: '2.0.0',
  role: Role.ANYONE,
  author: 'AjiroDesu',
  description: 'Fetch a random blue archive photo.',
  category: 'anime',
  usage: '',
  cooldown: 5,
  hasPrefix: true,
...
export const button = {
  [BUTTON_ID.refresh]: {
    ...
    onClick: async (ctx: AppCtx) => {
      await sendRandomBlueArchieve(ctx);
    },
  },
};
...
async function sendRandomBlueArchieve(ctx: AppCtx): Promise<void> {
  const { chat, native, event, button: btn } = ctx;
  const isButtonAction = event['type'] === 'button_action';

  try {
    const url = createUrl('nekolabs', '/random/blue-archive');
    ...
    const payload = {
      ...
      message: '**🖼️ Random Blue Archive Image**',

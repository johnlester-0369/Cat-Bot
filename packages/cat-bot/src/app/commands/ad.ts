/**
 * /ad — Generate an Ad Poster
 *
 * Fetches the target user's avatar (mention or self) and passes it to the
 * PopCat /v2/ad endpoint. The API returns an image which is sent as a
 * Buffer attachment.
 *
...
export const config: CommandConfig = {
  ...
  role: Role.ANYONE,
  author: 'AjiroDesu',
  description: "Put a user's avatar on an ad billboard.",
  category: 'fun',
  usage: ['[@user]', '(reply to a message)'],
  cooldown: 5,
  hasPrefix: true,
};
...
export const onCommand = async ({
  ...
  try {
    ...
    const base = createUrl('popcat', '/v2/ad');
    if (!base) throw new Error('Failed to build Ad API URL.');
    ...
    await chat.replyMessage({
      ...
      message: '📢 **Ad Poster**',
      attachment: [{ name: 'ad.png', stream: imageBuffer }],
    });
  } catch (err) {

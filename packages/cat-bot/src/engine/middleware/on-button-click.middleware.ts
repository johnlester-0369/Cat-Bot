 ...
 */
export const enforceButtonScope: MiddlewareFn<OnButtonClickCtx> =
  async function (ctx, next): Promise<void> {
    ...
    const withoutScope =
      tildeIdx === -1 ? localButtonId : localButtonId.slice(0, tildeIdx);
    const hashIdx = withoutScope.indexOf('#');
    const baseButtonId =
      hashIdx === -1 ? withoutScope : withoutScope.slice(0, hashIdx);
    const scopeUserId =
      tildeIdx === -1 ? null : localButtonId.slice(tildeIdx + 1);

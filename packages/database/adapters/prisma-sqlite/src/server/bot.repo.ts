...
export class BotRepo {
  async create(userId: string, sessionId: string, dto: CreateBotRequestDto): Promise<CreateBotResponseDto> {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      else if (credentials.platform === Platforms.Telegram) await tx.botCredentialTelegram.create({ data: { userId, platformId, sessionId, telegramToken: encrypt(credentials.telegramToken) } });
      else if (credentials.platform === Platforms.FacebookPage) await tx.botCredentialFacebookPage.create({ data: { userId, platformId, sessionId, fbAccessToken: encrypt(credentials.fbAccessToken), fbPageId: credentials.fbPageId } });
  ...
  async getById(userId: string, sessionId: string): Promise<GetBotDetailResponseDto | null> {
    ...
    if (platform === Platforms.Discord) {
      ...
      credentials = { platform: Platforms.Discord, discordToken: decrypt(cred.discordToken), discordClientId: cred.discordClientId };
    } else if (platform === Platforms.Telegram) {
      const cred = await prisma.botCredentialTelegram.findFirst({ where: { userId, sessionId } });
      if (!cred) throw new Error(`Missing credentials`);
      credentials = { platform: Platforms.Telegram, telegramToken: decrypt(cred.telegramToken) };
      ...
      if (!cred) throw new Error(`Missing credentials`);
      credentials = { platform: Platforms.FacebookPage, fbAccessToken: decrypt(cred.fbAccessToken), fbPageId: cred.fbPageId };
    } else {
  ...
  async update(userId: string, sessionId: string, dto: UpdateBotRequestDto, isCredentialsModified: boolean = false): Promise<void> {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      else if (credentials.platform === Platforms.Telegram) await tx.botCredentialTelegram.update({ where: { userId_platformId_sessionId: { userId, platformId, sessionId } }, data: { telegramToken: encrypt(credentials.telegramToken), ...(isCredentialsModified ? { isCommandRegister: false, commandHash: null } : {}) } });
      else if (credentials.platform === Platforms.FacebookPage) await tx.botCredentialFacebookPage.update({ where: { userId_platformId_sessionId: { userId, platformId, sessionId } }, data: { fbAccessToken: encrypt(credentials.fbAccessToken), fbPageId: credentials.fbPageId } });

...
export async function listSystemAdmins(): Promise<SystemAdminItem[]> {
  const res = await pool.query<{ id: string; admin_id: string; created_at: Date }>(
    `SELECT id, admin_id, created_at FROM system_admin ORDER BY created_at ASC`,
  );
  return res.rows.map((r) => ({ id: r.id, adminId: r.admin_id, createdAt: r.created_at.toISOString() }));
}
...
export async function addSystemAdmin(adminId: string): Promise<SystemAdminItem> {
  ...
  if (!row) throw new Error(`[system-admin] Failed to insert or find admin_id=${adminId}`);
  return { id: row.id, adminId: row.admin_id, createdAt: row.created_at.toISOString() };
}

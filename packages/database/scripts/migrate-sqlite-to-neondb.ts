...
async function main() {
  ...
  const[
    user, session, account, verification, botSession, botAdmin, botPremium, botCredentialDiscord, botCredentialTelegram,
    ...
    safeFind(prisma.botSession.findMany()), safeFind(prisma.botAdmin.findMany()), safeFind(prisma.botPremium.findMany()), safeFind(prisma.botCredentialDiscord.findMany()),
  ...
  try {
    ...
    try {
      await client.query(`TRUNCATE TABLE "user", bot_users, bot_threads, system_admin CASCADE`);
    } catch (e: any) {
    ...
    for (const def of tablesDef) {
      ...
      for (let i = 0; i < rows.length; i += 100) {
        ...
        try {
          ...
          await client.query(`INSERT INTO ${def.table} (${colNames.join(', ')}) VALUES ${placeholders.join(', ')} ON CONFLICT DO NOTHING`, values);

...
- [ ] `https://raw.githubusercontent.com/johnlester-0369/Cat-Bot/refs/heads/main/docs/llms.txt?v=9`
...
When `FCA_ENABLE_E2EE=true` (the default), `listenMqtt` spins up **two concurrent connections per session**: the standard MQTT connection to Facebook's edge servers, and a separate E2EE WebSocket handled by `FB-Messenger-E2EE` — a **pure JavaScript** implementation bundled inside `fca-cat-bot`. Unlike the previous `fca-unofficial-e2ee` library which spawned a Golang subprocess via `meta-messenger-js`, `fca-cat-bot` runs entirely within the Node.js process. Memory usage does increase per session when E2EE is enabled, but it remains **stable and predictable** — scaling on demand rather than growing unboundedly over time.

If you want to maximize performance or run many concurrent sessions on a resource-constrained server, consider setting `FCA_ENABLE_E2EE=false` in your `.env`. This reduces each session to a single MQTT connection and drops the E2EE bridge entirely. **However, disabling E2EE means the bot cannot respond to private messages (DMs) at all** — private Messenger chats use the E2EE channel, so without the bridge the bot receives no DM events and cannot reply to them. The bot will only function in group chats.

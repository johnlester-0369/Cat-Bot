...
### Core Context & APIs
- [The Context Object — AppCtx](#the-context-object--appctx)
- [Chat API](#chat-api)
- [State API — Conversation Flows](#state-api--conversation-flows)
- [Button API — Interactive Buttons](#button-api--interactive-buttons)
- [Database Collections](#database-collections)

### Advanced Context & Constants
- [session — Auto-Resolved Flow Context](#session--auto-resolved-flow-context)
- [args and options](#args-and-options)
- [Remaining Context Fields](#remaining-context-fields)
- [native — Session Identity](#native--session-identity)
- [Role](#role)
...
- [Native Platform Access](#native-platform-access)
- [Extending the Middleware Pipeline](#extending-the-middleware-pipeline)
- [Repos Reference — Database Cache Layer](#repos-reference--database-cache-layer)
- [Adapters Models Reference — Event & Data Structures](#adapters-models-reference--event--data-structures)

### Examples & Migration
- [Full Examples](#full-examples)
- [Migration Notes — From Global-Variable Bots](#migration-notes--from-global-variable-bots)
...
### CRITICAL: Multi-Instance Data Safety Rules
...
The `native` object appears in every handler and carries three fields that identify **which bot session and platform received this event**. These fields are completely separate from the identity of the person who sent the message.
...
**`native.userId` — the bot owner's dashboard account ID**

This is the better-auth account ID of the person who registered this bot session through the web dashboard. It is **not** the platform user ID of the person who sent the message.

For example: if Alice owns the bot account and Bob sends a message in Discord, `native.userId` is Alice's dashboard account ID while `event['senderID']` is Bob's Discord user ID. They are completely different strings used for completely different purposes.

`native.userId` is the first argument to every repo function and every `db` call because all bot data — credentials, sessions, user records, thread records — is scoped under the bot owner's account. Passing `event['senderID']` in its place would silently query a different owner's data.

**`native.platform` — the platform that dispatched this event**

A lowercase string identifying which messaging platform delivered this event. Possible values: `'discord'`, `'telegram'`, `'facebook-messenger'`, `'facebook-page'`.

Use it to branch on platform-specific behaviour at runtime. For commands that only support certain platforms, declare `config.platform` in the module config instead and let the engine enforce it automatically — no manual runtime check needed.

**`native.sessionId` — the bot instance identifier**

The ID assigned to a specific set of credentials registered in the dashboard. A single bot owner (`native.userId`) can run multiple simultaneous bot instances — for example, one Discord bot for a gaming server and a second Discord bot for a study group — each with its own `sessionId`.

Together, `(native.userId, native.platform, native.sessionId)` form the composite key that uniquely identifies one running bot instance. All database records for sessions, credentials, and per-thread/user data are stored under this triplet. Each bot instance sees only its own data even when multiple instances share the same database.

```ts
const { userId, platform, sessionId } = native

// ✅ Correct — scoped to this bot session's owner and instance
const admins = await listBotAdmins(userId, platform, sessionId)

// ❌ Wrong — event['senderID'] is who typed the message, not the bot owner
const admins = await listBotAdmins(event['senderID'] as string, platform, sessionId)
```

**Quick reference — `native` vs `event` identity fields:**

| Field | What it represents | Example value | Primary use |
|---|---|---|---|
| `native.userId` | Bot owner's dashboard account ID | `'cm3k7abc...'` | First argument to every repo / `db` call |
| `native.platform` | Platform that delivered this event | `'discord'` | Runtime platform branching; part of DB composite key |
| `native.sessionId` | This bot instance's identifier | `'gaming-server-bot'` | Third argument to every repo / `db` call |
| `event['senderID']` | Platform user ID of the message sender | `'987654321'` | Identifying who typed the message in chat |
...
> **Why use `hasNativeButtons`?** Platforms like Facebook Messenger do not support native UI buttons. If you provide a `button` array without conditionally checking `hasNativeButtons`, the engine (specifically inside `createChatContext` in `packages/cat-bot/src/engine/adapters/models/context.model.ts`) automatically falls back to generating a numbered text menu appended to your message and registers an internal `onReply` fallback state to route the user's numeric choice. Conditionally omitting the `button` array using `hasNativeButtons(native.platform)` ensures this text menu fallback isn't rendered, keeping standard text responses clean when native buttons are unsupported.
...
Middleware is a function that sits between the event system and your handler. The engine executes middleware functions as a chain in registration order. Each function receives the shared `ctx` object and a `next` callback: calling `next()` passes control to the next function in the chain (eventually reaching your handler); returning without calling `next()` halts the chain entirely — your handler never runs.

The built-in middleware chains run automatically before your handler on every event:

| Hook | Middleware (in order) | What it does |
|---|---|---|
| `onCommand` | `enforceNotBanned` | Silently drops commands from banned users or threads |
| | `enforcePermission` | Enforces `config.role`; rejects unauthorised callers |
| | `enforceCooldown` | Rate-limits per-user command invocations |
| | `validateCommandOptions` | Parses and validates `config.options` |
| `onChat` | `chatPassthrough` | Syncs thread and sender data to the database |
| | `chatLogThread` | Logs every incoming message to the session logger |
...
Use middleware for **cross-cutting concerns** — behaviour that applies to every invocation of a lifecycle hook regardless of which command or module triggered it:

- Global spam detection or IP-based rate limiting
- Feature flag enforcement that disables entire command categories
- Audit logging that sends every command invocation to an external service
- Organisation/tenant isolation for multi-tenant bot deployments
- Permission checks beyond what `config.role` provides (e.g. Discord server-role validation)

**When not to use middleware:**

Do not add logic to middleware that only applies to one command or a small set of commands. Every middleware function runs for every invocation of its lifecycle hook — adding single-command logic here wastes processing on every other command.

For single-command concerns, put the logic inside the `onCommand` handler behind an `if` guard or a helper function.
...
| `attachments` | `unknown[]` | Array of attachment objects — may be empty; narrow each item on `(att as {type:string}).type` (see Attachment Data Shapes) |
